FilterFieldExampleAssistant = Class.create( ExampleAssistantBase, {
	...
	printExamples : function(examples, match){
		...
		for(var i=0;i<examples.length;i++){
			this.filteredItems.insert('<div class=\"palm-body-text\">'+matchText(examples[i].name,match)+' - '+matchText(examples[i].type,match)+'</div>');
		}
	},
	filter: function(event) {
		...
		if(filterString){
			...
			console.log('number of matches == '+matching.length);
