...
amok.mock_all_with = function(mock) {
  return function() {
    return mock;
  }
};
...
amok.Mock = function(mockee) {
  ...
  this.addMethod = function(method) {
    ...
    //Add the method to the mock
    _methodIndex[method.getName()] = 0
    _methodCallCount[method.getName()] = 0
    this[method.getName()] = function (val) {
      var currentMethod = _methods[method.getName()][_methodIndex[method.getName()]];
      if (! currentMethod) {
        var argValues = "";
        for (var i = 0; i < arguments.length ; i++) {
          argValues += " ";
          argValues += arguments[i];
        }
        amok.Util.error("amok Method: unexpected call to " + method.getName() + " with" + argValues);
      }
      var returnVal = currentMethod.callback(arguments);
      if (currentMethod.hitCallLimit()) {
        _methodIndex[method.getName()]++;
      }
      _methodCallCount[method.getName()]++;
      return returnVal;
    }

    //Store the method in the methods array
    _methods[method.getName()] = _methods[method.getName()] || [];
    _methods[method.getName()].push(method);
    _liveMethod = method;

  };


  //The last method to I added to the mock
  this.getLiveMethod = function() {
    return _liveMethod;
  };

  this.getCallCountFor = function(methodName) {
    return _methodCallCount[methodName];
  }
};
...
amok.Method = function(name, expectation) {
  ...
  this.hitCallLimit = function() {
    return _expectation.getCallLimit() == null || _calls == _expectation.getCallLimit()
  }
  ...
  this.callback = function(args) {
    ...
    for (var i = 0; i < _expectation.getArguments().length; i++) {
      ...
      var equalityTest = amok.Util.equal(arg, args[i]);
      if (equalityTest.result === false) {
        amok.Util.error("amok Method: " + _name + "() has been called with an argument that differs from that expected:<br /> "+ equalityTest.message);
      }
    }
...
amok.Util = {
  ...
  error: function(msg) {
    ...
    if (amok.BROWSER_ERRORS) throw {name: 'amok error', message: msg};
  },
   ...
   */
  equal: function(a, b) {
    //TODO: refactor equality function
    //this is a guard clause that protects us from trying to call methods on an undefined object.
    //if a is undefined, b is either undefined and they match, or b is defined and they do not
    //in either case we don't need to test further.
    if (a === undefined || a === null) {
      if (a == undefined && b === undefined) {
        return {result: true};
      } else {
        return {result: false, message: 'Expected was undefined, actual was defined'};
      }
    }

    //Different type
    if (a._type != b._type) return {result: false, message: 'types are not equal, expected type was <b>'+ a._type + '</b>, actual type was <b>' + (b._type ? b._type : 'Object') + '</b>'};

    //Check array contents
    if (a._type == "Array") {
      if (a.length != b.length) return {result: false, message: 'Array lengths do not match. Expected length is ' + a.length + ' Actual is ' + b.length };
      for (var i = 0; i < a.length; i++) {
        if (!this.equal(a[i], b[i]).result) return {result: false, message: 'Array contents do not match at index ' + i + '. Expected value is ' + a[i] + ' Actual is ' + b[i] };
      }
      return {result: true};
    }

    if (a.constructor === HTMLDivElement) {
      if (a.id != b.id) return {result: false, message: 'Element ids do not match: Expected is ' + a.id + ' Actual is ' + b.id };
      return {result: true};
    }
    ...
    if (typeof a == "object") {
      for (property in a) {
        ...
        if (a[property] === undefined || b[property] === undefined || !this.equal(a[property], b[property]).result) {
          return {result:false, message: 'value of \'' + property + '\' on actual does not match value on expected'}; // beware! recursive craziness: on expected is '+ a[property] + ' does not match actual: ' + b[property]};
        }
      }
      for (property in b) {
        if (a[property] === undefined && b[property] === undefined) {
        ...
        if (b[property] === undefined || a[property] === undefined || !this.equal(a[property], b[property]).result) {
          return {result:false, message: 'value of \'' + property + '\' on expected does not match value on expected'};
        }
      }
    ...
    //Straight check
    return {result: (a === b), message: '<b>' + a + '</b> is not equal to <b>' + b + '</b>'};
  },
...
HTMLDivElement.prototype._type = "HTMLDivElement";
